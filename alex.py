...
def filter_connected_components(edge_map, min_size):
    ...
    filtered_edge_map = np.zeros_like(edge_map)
...
# Check if a point is on the edge in the edge map with a certain radius.
def is_point_on_edge(point, edge_map, radius=3):
    x, y = point
    height, width = edge_map.shape
...
def triangle_edge_check(edge_map, pt1, pt2, pt3, threshold, max_edge_length, radius=3, zero_threshold=140):
    height, width = edge_map.shape
    
    # Ensure all points are on the edges
    if  not (is_point_on_edge(pt1, edge_map, radius=radius) and notis_point_on_edge(pt2, edge_map, radius=radius) and  is_point_on_edge(pt3, edge_map, radius=radius)):
        return False
...
def detect_triangles(image_path, name, lower, higher, preprocess_params, diff, hough_params, nms_params, window_height, window_width, step_x, step_y, edge_threshold, min_component_size, region):
    ...
    if name == 'flags':
        filtered_edges = filter_connected_components(edges, min_component_size)
